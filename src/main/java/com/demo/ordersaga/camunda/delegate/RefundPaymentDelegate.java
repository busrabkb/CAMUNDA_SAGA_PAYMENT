...
public class RefundPaymentDelegate implements JavaDelegate {
    ...
    public RefundPaymentDelegate(PaymentService paymentService, CamundaVariableStore variableStore, TaskResponseJsonMapper responseJsonMapper) {
    ...
    public void execute(DelegateExecution execution) {
        ...
        String status = paymentService.refund(orderId, amount).name();
        execution.setVariable(ProcessVariables.PAYMENT_STATUS, status);
        variableStore.save(execution, ProcessVariables.REFUND_RESPONSE,
                responseJsonMapper.toJson(new RefundPaymentTaskResponse(status)));
    }
}
